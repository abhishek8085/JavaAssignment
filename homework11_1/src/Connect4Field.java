...
public class Connect4Field extends AbstractController implements Connect4FieldInterface, UserMoveListener {
    ...
    private Object scanMutex = new Object();
    private static volatile int scanCounter=0;
    private static boolean isLastMoveWin=false;
    ...
    public Connect4Field(Model model, int initialSize, int depth) {
        ...
        this.depth = depth;
        initializeField();
        new HorizontalScanThread().start();
        new VerticalScanThread().start();
        new LeftDiagonalScanThread().start();
        new RightDiagonalThread().start();
    }
 ...
 /*       System.out.println(scanHorizontalForWinMove() || scanVerticalForWinMove()
                || scanLeftDiagonalForWinMove()
                || scanRightDiagonalForWinMove());*/

        return scanForWinmove();



    }

    private boolean scanForWinmove()
    {
        scanCounter=0;
        isLastMoveWin=false;
        ...
        return isLastMoveWin;
    }

    class HorizontalScanThread extends Thread
    {
        @Override
        public void run() {
            while (true) {
                synchronized (scanMutex) {
                    ...
                    isLastMoveWin|=scanHorizontalForWinMove();
                }
            }
        }
    }

    class VerticalScanThread extends Thread
    {
        @Override
        public void run() {
            while (true) {
                synchronized (scanMutex) {
                    ...
                    incrementScanCounter();
                    isLastMoveWin|=scanVerticalForWinMove();
                }
            }
        }
    }

    class LeftDiagonalScanThread extends Thread
    {
        @Override
        public void run() {
            while (true) {
                synchronized (scanMutex) {
                    ...
                    incrementScanCounter();
                    isLastMoveWin|=scanLeftDiagonalForWinMove();
                }
            }
        }
    }

    public synchronized void incrementScanCounter()
    {
        scanCounter++;
    }

    class RightDiagonalThread extends Thread
    {
        @Override
        public void run() {
            while (true) {
                synchronized (scanMutex) {
                    ...
                    incrementScanCounter();
                    isLastMoveWin|=scanRightDiagonalForWinMove();
                }
            }
        }
    }
